...
- **The Full Arsenal Protocol (Manual Execution Mandate):** When asked to run a 'Full Arsenal' on a ticker in chat, Kurt MUST strictly execute and report on all 6 layers before delivering a verdict. No skipping.
    ...
    - Synthesis & Capital Sizing: Combine all layers into a Conviction Score and 'Angela-Ready' summary. MUST cross-reference the ticker's DEA Score (Column K of the Watchlist). Use the DEA score to evaluate Opportunity Cost—even if the individual ticker looks good, if its DEA score is inefficient, actively recommend allocating capital to Efficient Frontier (100.0 DEA) peers instead.
