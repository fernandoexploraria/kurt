...
### The JIT Transition-Matrix Execution Guard v2 (2026-07-23)
**Purpose:** `transition_matrix_live.py` is a just-in-time diagnostic that compares the most recent closed one-minute return-transition window with the same minute-of-session window from prior sessions. It reports whether the return-transition pattern is historically unusual and whether the anomaly is directionally adverse to the proposed order.

**Scope and Claim Discipline:** The diagnostic observes one-minute closing-price returns. It may report an anomalous return-transition regime, but it must not claim that it has identified institutional positioning, an order-book mutation, a falling knife, a blow-off top, or the cause of the change unless separate market-microstructure data supports that conclusion.

**Mandatory Invocation for Risk-Increasing Orders:** Immediately before a new long, add-to-long, or new short order, run the script with the applicable action and JSON output. Example:
...
2. `ALLOW_RISK_REDUCTION` / exit `0`: A warning may exist, but the requested action reduces risk. Proceed under the normal confirmation protocol.
3. `CAUTION` / exit `10`: Do not automatically increase risk. Present the analysis ID, anomaly percentile, directional metrics, and reasons to Fer; require explicit review or override.
4. `BLOCK_NEW_RISK` / exit `20`: Block the risk-increasing order. An override requires Fer's explicit confirmation and an audit record containing the analysis ID, timestamp, order details, override identity, and override reason.
5. `INSUFFICIENT_DATA`, `STALE_DATA`, `MARKET_CLOSED`, or `DATA_SOURCE_ERROR` / exits `30`, `31`, `32`, or `40`: Fail closed for risk-increasing orders. Do not treat a missing analysis as a green light. These failures do not, by themselves, block an urgent risk-reducing action.

**No Fixed JSD Rule:** The obsolete `JSD < 0.15 / JSD > 0.15` rule is removed. The script now uses matched historical windows, leave-one-session-out empirical calibration, a sequence-surprise measure, action-specific directional evidence, and minimum calibration requirements. JSD is an anomaly measurement, not the trade decision by itself.
